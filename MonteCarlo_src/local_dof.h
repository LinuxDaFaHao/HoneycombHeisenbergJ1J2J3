...
class LocalDOF {
 public:
  ...
  LocalDOF Project2XYPlane() const {
    assert(DimDof >= 2);
    LocalDOF res;
    res.spin_[0] = spin_[0];
    res.spin_[1] = spin_[1];
    for (size_t i = 2; i < DimDof; i++) {
      res.spin_[i] = 0.0;
    }
    return res;
  }

  /// e_z \cross Project2XYPlane()
  LocalDOF OrthogonalProject2XYPlane() const {
    assert(DimDof >= 2);
    LocalDOF res;
    res.spin_[0] = -spin_[1];
    res.spin_[1] = spin_[0];
  ...
  // cos(6\theta) in XY model
  double Cos6Theta() const {
    assert(DimDof == 2);
    const double &sx = spin_[0];
    const double &sy = spin_[1];
    double term1 = sx - sy;
    double term2 = sx + sy;
    double term3 = sx * sx - 4 * sx * sy + sy * sy;
    double term4 = sx * sx + 4 * sx * sy + sy * sy;
