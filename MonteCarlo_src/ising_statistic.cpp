...
*
* Description: Honeycomb lattice, the Ising model, summarize the results
*
*/
...
const size_t DimDof = 1; //heisenberg model
...
// anti-ferromagnetic susceptibility
  for (size_t i = 0; i < DimDof; i++) {
    ...
    res_.push_back(Variance((valid_data_of_magnetization_abs1)) / N * beta);
    res_.push_back(Variance((valid_data_of_magnetization_abs2)) / N * beta);
    res_.push_back(Variance((valid_data_of_magnetization_abs3)) / N * beta);
  }
...
//  res.push_back(0.0);
  ...
  res_.push_back(Variance(data_of_c3_order_parameter_abs) / N / N * beta); // susceptibility of C3 order parameter b
