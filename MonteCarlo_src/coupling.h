...
class CouplingStructure {
 ...
 public:
  ...
  CouplingStructure operator*(const CouplingStructure &rhs) const {
    CouplingStructure res;
    for (size_t i = 0; i < DimDof; i++) {
      for (size_t j = 0; j < DimDof; j++) {
        res.coupling_const[i][j] = 0.0;
        for (size_t k = 0; k < DimDof; k++) {
          res.coupling_const[i][j] += this->coupling_const[i][k] * rhs.coupling_const[k][j];
        }
      }
    }
    return res;
